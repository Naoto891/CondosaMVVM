...
class ContratoDao {
    ...
    suspend fun getImagenSolicitante(idContrato: Int, idSolicitante: Int): ByteArray? = dbQuery {
        ContratoTable
            .select { (ContratoTable.idContrato eq idContrato) and (ContratoTable.idSolicitante eq idSolicitante) }
            .singleOrNull()
            ?.get(ContratoTable.firmaSolicitante)
    }
